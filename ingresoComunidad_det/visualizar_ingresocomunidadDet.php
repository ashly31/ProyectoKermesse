...
if(isset($varIdICD))
{
    $varIdICD = $_GET['viewICD']; //RECUPERAMOS EL VALOR DE NUESTRA VARIABLE PARA EDITAR EL USUARIO
}

//OBTENEMOS LOS DATOS DEL USUARIO PARA SER CONSULTADO
$tk = $dticd->getIngresoCDByID($varIdICD);
?>
...
<head>
    ...
    <title>Kermesse - Ver Ingreso Comunidad Detalle</title>
...
<div id="wrapper">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div class="container-fluid">
            ...
            <!-- Page Heading -->
            <h1 class="h3 mb-0 text-gray-800">Ingreso Comunidad Detalle</h1>
            <p class="mb-4">En este formulario podrá ver los detalles de los ingresos de comunidad.
            </p>
            <!-- Agregar AC -->
            <div class="card mb-4">
                <div class="card-header">
                    <i class="fas fa-table me-1"></i>
                    Ver Ingreso Comunidad Detalles
                </div>
                <div class="card-body">
                    <form method="POST" action="../negocio/tbl_ingresocomunidad.php">
                        ...
                        <div class="form-floating mb-3">
                            <label for="id_ingreso_comunidad_det">ID Ingreso Comunidad Detalles</label>
                            <input class="form-control" id="id_ingreso_comunidad_det" name="id_ingreso_comunidad_det" readonly required/>
                        </div>

                        <div class="form-floating mb-3">
                            <label for="id_ingreso_comunidad">ID Ingreso Comunidad</label>
                            <input class="form-control" id="id_ingreso_comunidad" name="id_ingreso_comunidad" readonly required/>
                        </div>

                        <div class="form-floating mb-3">
                            <label for="id_bono">ID Bono</label>
                            <input class="form-control" id="id_bono" name="id_bono" readonly required/>
                        </div>

                        <div class="form-floating mb-3">
                            <label for="Denominacion">Denominacion</label>
                            <input class="form-control" id="Denominacion" name="Denominacion" type="text"  readonly required/>
                        </div>
                        <div class="form-floating mb-3">
                            <label for="cantidad">Cantidad</label>
                            <input class="form-control" id="cantidad" name="cantidad" type="number" readonly required/>
                        </div>
                        <div class="form-floating mb-3">
                            <label for="subtotal_bono">Subtotal Bonos</label>
                            <input class="form-control" id="subtotal_bono" name="subtotal_bono" type="number" readonly required/>
                        </div>

                        <div class="d-flex align-items-end justify-content-end mt-4 mb-0 gap-3">
                            <a href="../tbl_ingresoComunidad_det.php"> <button type="button" class="btn btn-info">Regresar</button> </a>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    function setValores()
    {
        ...
        $("#id_ingreso_comunidad_det").val("<?php echo $ticd->__GET('id_ingreso_comunidad_det') ?>");

        $("#id_ingreso_comunidad").css("background-color", "#E3E4E5");
        $("#id_ingreso_comunidad").val("<?php echo $ticd->__GET('id_ingreso_comunidad') ?>");

        $("#id_bono").css("background-color", "#E3E4E5");
        $("#id_bono").val("<?php echo $ticd->__GET('id_bono') ?>");

        $("#denominacion").css("background-color", "#E3E4E5");
        $("#denominacion").val("<?php echo $ticd->__GET('denominacion') ?>");

        $("#cantidad").css("background-color", "#E3E4E5");
        $("#cantidad").val("<?php echo $ticd->__GET('cantidad') ?>");

        $("#subtotal_bono").css("background-color", "#E3E4E5");
        $("#subtotal_bono").val("<?php echo $ticd->__GET('subtotal_bono') ?>");
